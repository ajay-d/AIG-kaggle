...
#RUN THIS CODE 4 times TO GET NN BAG 20. EACH RUN IS A BAG OF 5
...
while k<5:
    ...
    for train_index, test_index in skf.split(train_foo, labels):
        ...
        model = Sequential()
        model.add(Dense(1000, input_dim = train_foo.shape[1] , init='normal'))
...
dataset_blend_train.to_pickle('FEAT3_LVL1_NN2BAG5_TRAIN_' + str(np.mean(fin_score)) +'_.pkl') #nn1
dataset_blend_test.to_pickle('FEAT3_LVL1_NN2BAG5_TEST_' + str(np.mean(fin_score))+'_.pkl') # nn1
