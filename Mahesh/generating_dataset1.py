...
train.drop(['paid_i', 'paid_m','paid_l', 'paid_o', 'last_i', 'last_m', 'last_l', 'last_o', 
       'mc_string'], inplace = True, axis = 1)
...
foo2count = ['diagnosis_icd9_cd', 'cas_aia_cds_1_2', 'clm_aia_cds_1_2',
...
data['imparmt_pct'] = np.where(data['imparmt_pct']>100, (data['imparmt_pct'])/100, data['imparmt_pct'])
...
#TRYING AFTER A RUN OF RANDOM FOREST   #CV 0.24181 LB 0.25128
...
if updated_trainfoo:    
    train_foo = train[['econ_unemployment_py', 'slice', 'diagnosis_icd9_cd', 'surgery_yearmo',
       ...
       'initl_trtmt_cd', 'catas_or_jntcvg_cd', 'sic_cd', 'clmnt_birth_yearmo',
       'empl_hire_yearmo', 'death_yearmo', 'loss_yearmo', 'reported_yearmo',
       'mmi_yearmo', 'rtrn_to_wrk_yearmo', 'state', 'occ_code', 'occ_desc1',
       'occ_desc2', 'suit_yearmo', 'suit_matter_type', 'law_limit_tt',
       'law_limit_pt', 'law_limit_pp', 'law_cola', 'law_offsets',
       'law_ib_scheduled', 'econ_gdp_ly', 'econ_gdp_ar_ly', 'econ_price_ly',
...
train_foo.to_pickle("train_foo.pkl")
final_test_foo.to_pickle("final_test_foo.pkl")
