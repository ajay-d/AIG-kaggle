...
col2scale = ['econ_unemployment_py', 'diagnosis_icd9_cd',
       'avg_wkly_wage','cas_aia_cds_1_2', 'clm_aia_cds_1_2', 'clm_aia_cds_3_4',
       'imparmt_pct', 'sic_cd', 'occ_code', 'occ_desc1', 'occ_desc2','econ_gdp_ly',
       'econ_gdp_ar_ly', 'econ_price_ly', 'econ_price_allitems_ly',
       'econ_price_healthcare_ly', 'econ_10yr_note_ly',
       'econ_unemployment_ly', 'diff_repo_loss',
       ...
       'age_claimaint','target_prev']

train_foo['train_flag'] = 1
test_foo['train_flag'] = 0
ttdata = train_foo.append(test_foo)

col2dummy= ['initl_trtmt_cd','slice','surgery_yearmo','clmnt_gender','imparmt_pct_yearmo', 'catas_or_jntcvg_cd', 
            'clmnt_birth_yearmo', 'empl_hire_yearmo', 'death_yearmo', 'loss_yearmo', 'reported_yearmo',
       'mmi_yearmo', 'rtrn_to_wrk_yearmo', 'state', 'suit_yearmo', 'suit_matter_type', 'law_limit_tt',
       'law_limit_pt', 'law_limit_pp', 'law_cola', 'law_offsets',
       'law_ib_scheduled', 'cutoff_yr','adj_prev', 'cutoffyr_flag']
...
while k<10:
    ...
    for train_index, test_index in skf.split(train_foo, labels):
        ...
        model = Sequential()
        model.add(Dense(1000, input_dim = 510 , init='normal')) #try init = uniform  510
        model.add(Activation('relu'))
        #model.add(BatchNormalization())
        model.add(Dropout(0.1))
        model.add(Dense(500, init='normal'))
        model.add(Activation('relu'))     #model.add(PReLU())
        model.add(Dropout(0.1))
...
dataset_blend_train.to_pickle('NN_BAG10_TRAIN_CV_' + str(np.mean(fin_score)) +'_.pkl')
dataset_blend_test.to_pickle('NN_BAG10_TEST_CV_' + str(np.mean(fin_score))+'_.pkl')
    
print ("Done with bag of 10 NNs------>")


#BAG OF 10 NNs -2nd run
...
dataset_blend_train.to_pickle('NN_2_BAG10_TRAIN_CV_' + str(np.mean(fin_score)) +'_.pkl')
dataset_blend_test.to_pickle('NN_2_BAG10_TEST_CV_' + str(np.mean(fin_score))+'_.pkl')
