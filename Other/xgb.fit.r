...
xgb.fit <- function(working.train.test,split,train.fold,valid.fold){
  ...
  working.train.test[is.na(working.train.test)] <- NA

  # X <- select(working.train.test,-claim_id,-target,-split)
  # m <- apply(X,2,function(x)mean(x,na.rm=TRUE))
  # s <- apply(X,2,function(x)sd(x,na.rm=TRUE))
  ...
  # X.train <- (X.train-matrix(rep(m,nrow(X.train)),nrow=nrow(X.train),byrow=TRUE))/matrix(rep(s,nrow(X.train)),nrow=nrow(X.train),byrow=TRUE)
  # X.valid <- (X.valid-matrix(rep(m,nrow(X.valid)),nrow=nrow(X.valid),byrow=TRUE))/matrix(rep(s,nrow(X.valid)),nrow=nrow(X.valid),byrow=TRUE)
  # X.test <- (X.test-matrix(rep(m,nrow(X.test)),nrow=nrow(X.test),byrow=TRUE))/matrix(rep(s,nrow(X.test)),nrow=nrow(X.test),byrow=TRUE)
