...
X, X_test, Y, Y_test = train_test_split(X.values, to_categorical(y_train), test_size=0.15, random_state=42)

model = Sequential()
model.add(Dense(100, input_dim=X.shape[1]))
model.add(Dense(21, init = 'glorot_uniform', activation='softmax'))
model.compile(optimizer='sgd', loss='categorical_crossentropy')
model.fit(X.values, Y, batch_size=10000, 
          nb_epoch=5, shuffle=True)

fBestModel = 'best_model.h5' 
early_stop = EarlyStopping(monitor='val_loss', patience=4, verbose=1) 
best_model = ModelCheckpoint(fBestModel, verbose=0, save_best_only=True)
model.fit(X, Y, validation_data = (X_test, Y_test), nb_epoch=5, 
          batch_size=5000, verbose=True, validation_split=0.15, 
