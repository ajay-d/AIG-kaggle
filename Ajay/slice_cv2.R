...
#  mutate(cat.number = ifelse(var %in% c("prexist_dsblty_in", "catas_or_jntcvg_cd", "suit_matter_type", "initl_trtmt_cd", "major_class_cd", "state"), row_number()-1, cat.number)) %>%
#  fill(cat.number)
...
train.file <- "data/train_recode_2.csv.gz"
write.csv(train.data, gzfile(train.file), row.names=FALSE)

test.file <- "data/test_recode_2.csv.gz"
...
write.csv(gbm.1, gzfile("submission/gbm_slice2_cv1.csv.gz"), row.names=FALSE)
#######################GBM 2 - deeper########################################################
...
gbm.2 <- cbind(test.data$claim_id, gbm.2) %>%
  as.data.frame() %>%
  setNames(names(sample))
write.csv(gbm.2, gzfile("submission/gbm_slice2_cv2.csv.gz"), row.names=FALSE)
