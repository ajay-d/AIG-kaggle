'''
Based on keras_10_models_02
'''
from __future__ import division
version = 'keras_10_models_02_all'
...
claim.diagnosis_icd9_cd[claim.diagnosis_icd9_cd.isnull()] = '_M'
...
var_lag = ['report_lag']
var_code = ['diagnosis_icd9_cd','cas_aia_cds_1_2','cas_aia_cds_3_4','clm_aia_cds_1_2','clm_aia_cds_3_4',\
            'sic_cd','occ_code']
...
for sl in range(1,11,1):
    ...
    train_test = pd.merge(train_test, claim_id, how='inner', on=['claim_id'])
    train_test.split.value_counts()
    
    train_test['slice_ind'] = np.where(train_test.split>=1, sl, train_test.slice)
    
    train_test_id = train_test[['claim_id']][(train_test.paid_year == 2015 - train_test.slice_ind)&(train_test.slice_ind == sl)]
    train_test_id.drop_duplicates(['claim_id'], inplace=True)
    
    train_test = pd.merge(train_test, train_test_id, on=['claim_id'], how='inner')
    del(train_test_id)
    
    print('>>> creating years and lags')
    train_test['cutoff_year'] = train_test.paid_year - train_test.slice_ind
    ...
    train_test['mmi_cutoff_lag'] = np.where(train_test.mmi_yearmo.isnull(), np.nan, (2015 - train_test.slice_ind) - train_test.mmi_yearmo) 
    train_test['surgury_cutoff_lag'] = np.where(train_test.surgery_yearmo.isnull(), np.nan, (2015 - train_test.slice_ind) - train_test.surgery_yearmo) 
    train_test['imparmt_cutoff_lag'] = np.where(train_test.imparmt_pct_yearmo.isnull(), np.nan, (2015 - train_test.slice_ind) - train_test.imparmt_pct_yearmo) 
    ...
    first_paid = train_test[['claim_id','paid_year']][(2015 - train_test.slice_ind) >= train_test.paid_year]
    ...
    first_non0_paid = train_test[['claim_id','paid_year','paid_m','paid_i']][(2015 - train_test.slice_ind) >= train_test.paid_year]
    ...
    last_non0_paid = train_test[['claim_id','paid_year','paid_m','paid_i']][(2015 - train_test.slice_ind) >= train_test.paid_year]
    ...
    mi_paid = train_test[['claim_id', 'paid_m', 'paid_i', 'paid_l', 'paid_o']][(2015 - train_test.slice_ind) >= train_test.paid_year]
    ...
    mi_paid = train_test[['claim_id', 'paid_m', 'paid_i', 'paid_l', 'paid_o','adj']][(2015 - train_test.slice_ind) >= train_test.paid_year]
    mi_paid['paid_mi'] =  mi_paid.paid_m + mi_paid.paid_i
    mi_paid['paid_milo'] =  mi_paid.paid_m + mi_paid.paid_i + mi_paid.paid_l + mi_paid.paid_o
    ...
    train_test_2015['paid_m_non0_2_date_prop'] = np.where(((2015 - train_test_2015.slice_ind) - train_test_2015.first_paid_year)>0, train_test_2015.paid_m_non0_2_date/((2015 - train_test_2015.slice_ind) - train_test_2015.first_paid_year), -1)
    train_test_2015['paid_i_non0_2_date_prop'] = np.where(((2015 - train_test_2015.slice_ind) - train_test_2015.first_paid_year)>0, train_test_2015.paid_i_non0_2_date/((2015 - train_test_2015.slice_ind) - train_test_2015.first_paid_year), -1)
    train_test_2015['paid_l_non0_2_date_prop'] = np.where(((2015 - train_test_2015.slice_ind) - train_test_2015.first_paid_year)>0, train_test_2015.paid_l_non0_2_date/((2015 - train_test_2015.slice_ind) - train_test_2015.first_paid_year), -1)
    train_test_2015['paid_o_non0_2_date_prop'] = np.where(((2015 - train_test_2015.slice_ind) - train_test_2015.first_paid_year)>0, train_test_2015.paid_o_non0_2_date/((2015 - train_test_2015.slice_ind) - train_test_2015.first_paid_year), -1)
    train_test_2015['paid_mi_non0_2_date_prop'] = np.where(((2015 - train_test_2015.slice_ind) - train_test_2015.first_paid_year)>0, train_test_2015.paid_mi_non0_2_date/((2015 - train_test_2015.slice_ind) - train_test_2015.first_paid_year), -1)
    train_test_2015['paid_milo_non0_2_date_prop'] = np.where(((2015 - train_test_2015.slice_ind) - train_test_2015.first_paid_year)>0, train_test_2015.paid_milo_non0_2_date/((2015 - train_test_2015.slice_ind) - train_test_2015.first_paid_year), -1)

    print('>>> prior 10 medical and indemnity payment')
    prior_10 = train_test[['claim_id', 'paid_m','paid_i', 'paid_l', 'paid_o']][((2015 - train_test.slice_ind) >= train_test.paid_year) & ((2015 - train_test.slice_ind) < (train_test.paid_year+10))]
    ...
    prior_5 = train_test[['claim_id', 'paid_m','paid_i', 'paid_l', 'paid_o']][((2015 - train_test.slice_ind) >= train_test.paid_year) & ((2015 - train_test.slice_ind) < (train_test.paid_year+5))]
    ...
    prior_3 = train_test[['claim_id', 'paid_m','paid_i', 'paid_l', 'paid_o']][((2015 - train_test.slice_ind) >= train_test.paid_year) & ((2015 - train_test.slice_ind) < (train_test.paid_year+3))]
    ...
    for i in [1,2,3,4,5,6]:
        prior = train_test[['claim_id', 'paid_m','paid_i', 'paid_l', 'paid_o']][((2015 - train_test.slice_ind) == (train_test.paid_year+i))]
    ...
    mc_string = train_test[['claim_id', 'mc_string']][(2015 - train_test.slice_ind) >= train_test.paid_year]
    ...
    cutoff_table = train_test[['claim_id', 'last_m', 'last_i', 'last_l', 'last_o', 'econ_unemployment_py', 'adj', 'paid_m', 'paid_i', 'paid_o', 'paid_l']][(2015 - train_test.slice_ind) == train_test.paid_year].add_suffix('_cutoff')
    ...
    train_test_2015['diff_yr_last0_cutoff'] = (2015 - train_test_2015.slice_ind) - train_test_2015.last_non0_paid_yr
    ...
    fee = ['paid_i','paid_m','paid_l','paid_o','last_i','last_m','last_l','last_o','adj','mc_string','slice','slice_ind']
    train_test_2015.drop(others + econ + fee, 1, inplace=True)
        
    x_tv = train_test_2015[train_test_2015.split>=0].reset_index(drop=True)
    x_test = train_test_2015[train_test_2015.split==-1].reset_index(drop=True)
    ...
    y_tv = pd.cut(x_tv.target, bins, labels=bins_name)

    ntv = x_tv.shape[0]

    train_test_2015 = x_tv.append(x_test)

    del(x_tv, x_test)

    train_test_2015.drop(['claim_id', 'target', 'split'], 1, inplace=True)

    print('>>> Preprocessing and transforming to sparse data')
    var_interaction = ['bp_cause_code','ic_cause_code','cause_detri_code', 'bp_detri_code', 'ic_detri_code',\
                   'state_wage','occ_ic','occ_bp','state_bp','state_ic','state_cause','state_detri',\
                  'state_icd9', 'bp_icd9','ic_icd9','cause_icd9','detri_icd9','state_adj_last','state_adj_cutoff',\
                  'state_suit_type','suit_type_yr', \
                  'last_m_last__last_non0_paid_yr', 'last_milo_last__last_non0_paid_yr','last_i_last__last_non0_paid_yr',\
                  'detri_icd9__last_non0_paid_yr']

    for var in var_interaction:
    ...
    for var in cats:
        dummy = pd.get_dummies(train_test_2015[var].astype('category'))
        tmp = csr_matrix(dummy)
        sparse_data.append(tmp)

    nums = train_test_2015.dtypes[train_test_2015.dtypes!='object'].index
    for var in nums:
        if (sum(train_test_2015[var].isnull()) > 0) | (sum(train_test_2015[var] == -np.inf) > 0) | (sum(train_test_2015[var] == np.inf) > 0):
            train_test_2015[var][train_test_2015[var].isnull()] = -1
            train_test_2015[var][train_test_2015[var] == -np.inf] = -1
            train_test_2015[var][train_test_2015[var] == np.inf] = -1
            
    scaler = StandardScaler()
    tmp = csr_matrix(scaler.fit_transform(train_test_2015[nums]))
    sparse_data.append(tmp)

    print('>>> sparse train and test data')
    xtrain_test_2015 = hstack(sparse_data, format = 'csr')
    xtv = xtrain_test_2015[:ntv, :]
    xtest = xtrain_test_2015[ntv:, :]


    del(xtrain_test_2015, sparse_data, tmp)

    def nn_model():
        model = Sequential()

        model.add(Dense(200, input_dim = xtv.shape[1], init = 'normal'))
        model.add(PReLU())
    ...
    nbags = 3
    nepochs = 25
    pred_test = np.zeros((xtest.shape[0],21))

    earlyStopping=EarlyStopping(monitor='val_loss', patience=2, verbose=1, mode='auto')

    for fold in range(nfolds):
        print('fold {0}'.format(fold))
        x_tr, x_val, y_tr, y_val = train_test_split(xtv, y_tv, train_size=.9, random_state=10*(fold+1))
        for j in range(nbags):
            print('bag {0}'.format(j))
            model = nn_model()
            fit = model.fit_generator(generator = batch_generator(x_tr, y_tr, 128, True),
                                      nb_epoch = nepochs,
    ...
    if sl == 1:
        pred_test_combine = np.c_[id_test, pred_test]
    else:
        pred_test_combine = np.r_[pred_test_combine, np.c_[id_test, pred_test]]
