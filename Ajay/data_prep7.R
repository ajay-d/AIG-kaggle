...
i <- 9
for(i1 in 1:(length(cat.vars)-1))
  for(i2 in (i1+1):length(cat.vars)) {
...
test.data <- test.data %>%
  mutate(block = row_number() %% 10)

train.data <- train.data %>%
  mutate(block = row_number() %% 10)

write_feather(train.data, "data/train.feather")
write_feather(test.data, "data/test.feather")
...
for (blk in 0:9) {
  ...
  write_feather(tr, paste0("data/train_", blk, ".feather"))
  write_feather(te, paste0("data/test_", blk, ".feather"))
}
...
write.csv(gbm.1, gzfile("submission/gbm_slice7_cv1.csv.gz"), row.names=FALSE)
