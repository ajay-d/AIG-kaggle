...
train.3 <- train.data %>%
  ...
  mutate(suit_yearmo = ifelse((cutoff_year - trunc(suit_yearmo)) < 0, NA, suit_yearmo),
         ...
         cat_1 = paste0(clm_aia_cds_1_2, clm_aia_cds_3_4, state),
         cat_2 = paste0(diagnosis_icd9_cd, catas_or_jntcvg_cd, state),
         cat_3 = paste0(diagnosis_icd9_cd, clm_aia_cds_1_2, clm_aia_cds_3_4),
         cat_4 = paste0(catas_or_jntcvg_cd, clm_aia_cds_1_2, clm_aia_cds_3_4),

         cat_5 = paste0(diagnosis_icd9_cd, state, clm_aia_cds_1_2, clm_aia_cds_3_4),
         cat_6 = paste0(state, catas_or_jntcvg_cd, clm_aia_cds_1_2, clm_aia_cds_3_4),
         cat_7 = paste0(diagnosis_icd9_cd, catas_or_jntcvg_cd, clm_aia_cds_1_2, clm_aia_cds_3_4),

         cat_8 = paste0(diagnosis_icd9_cd, catas_or_jntcvg_cd, clm_aia_cds_1_2, clm_aia_cds_3_4, state)
...
length(cat.vars)
cross.cat.vars <- c("clmnt_gender", "major_class_cd", "prexist_dsblty_in", "catas_or_jntcvg_cd",
                     "suit_matter_type", "initl_trtmt_cd", "state",
                     "occ_code", "sic_cd", "diagnosis_icd9_cd", "cas_aia_cds_1_2", "cas_aia_cds_3_4", "clm_aia_cds_1_2", "clm_aia_cds_3_4")
train.3a <- train.3 %>% select(claim_id)
length(cross.cat.vars)
choose(length(cross.cat.vars), 2)

i <- 9
for (i1 in 1:(length(cross.cat.vars) - 1))
  for (i2 in (i1 + 1):length(cross.cat.vars)) {

    j1 <- cross.cat.vars[[i1]]
    j2 <- cross.cat.vars[[i2]]

    new_var_train <- train.3 %>%
      select_('claim_id', j1, j2) %>%
      setNames(c('claim_id', 'var_1', 'var_2')) %>%
      mutate(new_cat = paste0(var_1, var_2)) %>%
      select(claim_id, new_cat) %>%
      setNames(c('claim_id', paste0('cat_', i)))
...
cnt.vars <- c(adj.vars, 'n_hist', 'any_i', 'any_m', 'any_l', 'any_o', 'sign_paid_i', 'sign_paid_m', 'sign_paid_im', 'slice')
...
train.file <- "data/train_recode_9.csv.gz"
write.csv(train.data, gzfile(train.file), row.names=FALSE)

test.file <- "data/test_recode_9.csv.gz"
